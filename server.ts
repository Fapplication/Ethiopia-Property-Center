...
import { users, properties, brokers, agents, inquiries, appointments, chatMessages, subscriptions, appSettings, auditLogs } from "./src/db/schema.ts";
    ...
    try {
      ...
      if (updated.length > 0) {
        await db.insert(auditLogs).values({
          ...
          details: `Broker: "${updated[0].companyName}" status: ${updated[0].status}`,
          timestamp: new Date(),
          user: 'Admin'
        });
        res.json(updated[0]);
