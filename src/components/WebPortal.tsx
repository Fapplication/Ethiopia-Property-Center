...
import { Search, MapPin, BedDouble, Bath, Square, Sparkles, SlidersHorizontal, Info, Compass, ArrowRightLeft, Calendar, User, Building2, Check, Phone, Mail, HelpCircle, Eye, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function WebPortal({
  ...
  const handleSubmitOwnerProperty = (e: React.FormEvent) => {
    ...
    onSubmitProperty({
      ...newPropForm,
      status: 'pending_review', // Owner submissions must be approved by broker
      isPremium: false,
      isFeatured: false,
    });
    alert("Listing submitted successfully! It has been routed to the selected broker's dashboard for verification before public release.");
            ...
            >
              ...
              <form onSubmit={handleSubmitOwnerProperty} className="space-y-4 text-xs">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block font-bold text-slate-600 mb-1">Assign Broker Reviewer</label>
                    <select
